-- Update Accept Invitation Function for Supabase Auth Integration
...
-- ============================================================================
-- Update function to accept Supabase Auth user ID and create proper user record
...
BEGIN
  ...
  -- Create admin user using the Supabase Auth user ID
  INSERT INTO users (
    id,
    tenant_id,
    email,
...
BEGIN
  ...
  -- Check if user already exists in this tenant (by email or auth user ID)
  IF EXISTS (
    SELECT 1 FROM users 
  ...
  -- Create user account using the Supabase Auth user ID
  INSERT INTO users (
    id,
    tenant_id,
    email,
...
COMMENT ON FUNCTION accept_invitation IS 
  'Atomically accepts invitation and creates user account with Supabase Auth integration';