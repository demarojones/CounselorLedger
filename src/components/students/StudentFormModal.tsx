...
export function StudentFormModal({ open, onOpenChange, student, onSuccess }: StudentFormModalProps) {
  ...
  const validate = (): boolean => {
    ...
    if (formData.studentId && formData.studentId.length > 20) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const submissionData = {
      studentId: formData.studentId || undefined, // Auto-generated by API if not provided
        ...
        <DialogBody className="overflow-y-auto">
          <form onSubmit={handleSubmit} className="space-y-6 py-2">
            ...
            <div className="space-y-1.5">
              <FormInput
                label="Student ID"
                placeholder="e.g., S12345 (auto-generated if left blank)"
                value={formData.studentId}
                onChange={e => handleChange('studentId', e.target.value)}
                error={errors.studentId}
              />
              <p className="text-xs text-muted-foreground">
                Leave blank to auto-generate a unique identifier.
              </p>
            </div>
